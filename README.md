-- nuclear.hook v2.3 | Keys: 123 x5 | Menu: Right Alt/Right Shift | HUD: Always On | Supports M1-M4 Binds
-- + Aim Part dropdown | + Bindable keys for ALL features | + Persistent keybind HUD
-- + Anti Drop-Off + Instant Hit + Hit Chance + Rage Bot | + Inventory Overlay
...
local Theme = {
	...
	Top = Color3.fromRGB(15, 13, 26),
	Card = Color3.fromRGB(21, 19, 36),
	Accent = Color3.fromRGB(168, 70, 255),
	Text = Color3.fromRGB(238, 230, 255),
	Dim = Color3.fromRGB(140, 130, 175),
...
KeyBtn.Position = UDim2.new(0.09, 0, 0.72, 0)
...
KeyHint.Position = UDim2.new(0, 0, 0.88, 0)
KeyHint.BackgroundTransparency = 1
KeyHint.Text = "5 keys registered"
KeyHint.TextColor3 = Color3.fromRGB(70, 200, 120)
...
Ver.Position = UDim2.new(0, 130, 0, 0)
...
TabBar.Size = UDim2.new(1, -230, 1, 0)
TabBar.Position = UDim2.new(0, 190, 0, 0)
...
local ESP = MakeTab("ESP")
local World = MakeTab("World")
local Misc = MakeTab("Misc")
local Config = MakeTab("Config")
local Settings = MakeTab("Settings")

Tabs["Rage"].Btn.BackgroundColor3 = Theme.Accent
Tabs["Rage"].Btn.TextColor3 = Color3.new(1, 1, 1)
Tabs["Rage"].Page.Visible = true
Current = "Rage"
...
local function Toggle(parent, text, default, cb)
	...
	t.BackgroundColor3 = on and Theme.Accent or Color3.fromRGB(42, 34, 60)
	t.Text = ""
	...
	t.MouseButton1Click:Connect(function()
		on = not on
		TweenService:Create(t, TweenInfo.new(0.12), { BackgroundColor3 = on and Theme.Accent or Color3.fromRGB(42, 34, 60) }):Play()
...
local function Slider(parent, text, min, max, default, suffix, cb)
	...
	track.BackgroundColor3 = Color3.fromRGB(34, 30, 52)
	track.Parent = f
...
-- KEYBIND SYSTEM (Keyboard + Mouse1-4)
local Binds = {
	Silent    = Enum.UserInputType.MouseButton1,
	Lock      = Enum.UserInputType.MouseButton2,
	Smooth    = Enum.KeyCode.LeftAlt,
...
local function ToggleWithBind(parent, text, default, bindName, cb)
	...
	bindBtn.BackgroundColor3 = Color3.fromRGB(50, 42, 75)
...
-- DROPDOWN SYSTEM (popup list, escapes ScrollingFrame clipping)
local OpenPopups = {}
...
local function Dropdown(parent, text, options, defaultIdx, cb)
	...
	b.BackgroundColor3 = Color3.fromRGB(50, 42, 75)
	...
	local function OpenPopup()
		...
		popupGui.Name = "NuclearHookDropdown"
		...
		for i, opt in ipairs(options) do
			...
			row.BackgroundColor3 = (i == idx) and Color3.fromRGB(60, 48, 95) or Color3.fromRGB(21, 19, 36)
			row.Text = opt
			row.TextColor3 = (i == idx) and Theme.Accent or Theme.Text
...
ToggleWithBind(Rage, "Silent Aim", false, "Silent", function(v)
	_G.SilentAim = v
end)

ToggleWithBind(Rage, "Lock Aim", false, "Lock", function(v)
	_G.LockAim = v
end)

ToggleWithBind(Rage, "Smooth Aim", false, "Smooth", function(v)
	_G.SmoothAim = v
end)

ToggleWithBind(Rage, "Triggerbot (Visible Only)", false, "Trigger", function(v)
	_G.Triggerbot = v
end)

ToggleWithBind(Rage, "Spinbot", false, "Spin", function(v)
	_G.Spinbot = v
end)

ToggleWithBind(Rage, "Hitbox Expander", false, "Hitbox", function(v)
	_G.HitboxExpander = v
end)

ToggleWithBind(Rage, "Prediction", false, "Pred", function(v)
	_G.Prediction = v
end)

Section(Rage, "Aim Enhancements")

ToggleWithBind(Rage, "Anti Drop-Off", false, "AntiDrop", function(v)
	_G.AntiDropOff = v
end)

Slider(Rage, "Drop-Off Timeout (ms)", 100, 2000, 800, "ms", function(v)
	_G.DropOffTimeout = v / 1000
end)

ToggleWithBind(Rage, "Instant Hit", false, "InstantHit", function(v)
	_G.InstantHit = v
end)

Slider(Rage, "Hit Chance %", 1, 100, 100, "%", function(v)
	_G.HitChance = v
end)

ToggleWithBind(Rage, "Rage Bot", false, "RageBot", function(v)
	_G.RageBot = v
end)

Slider(Rage, "Rage Fire Rate (ms)", 50, 500, 100, "ms", function(v)
	_G.RageFireRate = v / 1000
end)

Section(Rage, "Aim Settings")
Toggle(Rage, "Show FOV Circle", false, function(v) _G.ShowFOV = v end)
Slider(Rage, "Aim FOV (px)", 20, 400, 140, "", function(v) _G.AimFOV = v end)
local AimPartDrop = Dropdown(Rage, "Aim Part", AimPartList, 1, function(part)
	_G.AimPart = part
end)

Section(Legit, "Filters")
Toggle(Legit, "Team Check", true, function(v) _G.TeamCheck = v end)
Toggle(Legit, "Visibility Check", true, function(v) _G.VisCheck = v end)

Section(ESP, "ESP Features")
Toggle(ESP, "Master ESP", false, function(v)
	_G.MasterESP = v
	_G.ESP_Box = v
	_G.ESP_Highlight = v
	_G.ESP_Names = v
...
Toggle(ESP, "Box", false, function(v) _G.ESP_Box = v end)
Toggle(ESP, "Highlight / Chams", false, function(v) _G.ESP_Highlight = v end)
Toggle(ESP, "Name Tags", false, function(v) _G.ESP_Names = v end)
Toggle(ESP, "Health Bar", false, function(v) _G.ESP_Health = v end)
Toggle(ESP, "Distance", false, function(v) _G.ESP_Distance = v end)
Toggle(ESP, "Tracer Lines", false, function(v) _G.ESP_Tracer = v end)
Note(ESP, "ESP shows ALL players (through walls)")
...
Toggle(Misc, "Fly", false, function(v) _G.Fly = v end)
Slider(Misc, "Fly Speed", 10, 300, 60, "", function(v) _G.FlySpeed = v end)
Toggle(Misc, "Noclip", false, function(v) _G.Noclip = v end)
Toggle(Misc, "Infinite Jump", false, function(v) _G.InfJump = v end)
Toggle(Misc, "God Mode", false, function(v)
	_G.GodMode = v
...
MakeBtn(Config, "Save Config", function()
	savedConfigs[ConfigName.Text] = {
		SilentAim = _G.SilentAim, LockAim = _G.LockAim, SmoothAim = _G.SmoothAim,
		Triggerbot = _G.Triggerbot, Spinbot = _G.Spinbot, HitboxExpander = _G.HitboxExpander,
		MasterESP = _G.MasterESP, ESP_Box = _G.ESP_Box, ESP_Highlight = _G.ESP_Highlight,
		ESP_Names = _G.ESP_Names, ESP_Health = _G.ESP_Health, ESP_Distance = _G.ESP_Distance,
		ESP_Tracer = _G.ESP_Tracer, Fly = _G.Fly, Noclip = _G.Noclip, InfJump = _G.InfJump,
		FlySpeed = _G.FlySpeed, AimFOV = _G.AimFOV, Prediction = _G.Prediction,
		AimPart = _G.AimPart,
...
MakeBtn(Config, "Load Config", function()
...
Note(Settings, "Menu Toggle: Right Alt / Right Shift")
Note(Settings, "HUD: Always On Screen")
Note(Settings, "Key: 123 (x5)")
Note(Settings, "Supports Mouse1-4 + Keyboard binds")

-- ============ PERSISTENT KEYBIND HUD (never hides) ============
local HudGui = Instance.new("ScreenGui")
HudGui.Name = "NuclearHookBinds"
HudGui.ResetOnSpawn = false
HudGui.Parent = LocalPlayer:WaitForChild("PlayerGui")

local Hud = Instance.new("Frame")
Hud.Size = UDim2.new(0, 280, 0, 380)
Hud.Position = UDim2.new(0, 16, 1, -396)
Hud.BackgroundColor3 = Color3.fromRGB(13, 12, 20)
...
HudStroke.Color = Color3.fromRGB(125, 95, 195)
...
HudTitle.Text = "NUCLEAR.HOOK v2.3  -  KEYBINDS"
...
local HudUnderline = Instance.new("Frame")
HudUnderline.Size = UDim2.new(1, 0, 0, 1)
HudUnderline.BackgroundColor3 = Theme.Accent
HudUnderline.BackgroundTransparency = 0.55
HudUnderline.BorderSizePixel = 0
HudUnderline.Parent = Hud

local HudDefs = {
	{ Key = "Silent",    Label = "Silent Aim" },
	{ Key = "Lock",      Label = "Lock Aim" },
	{ Key = "Smooth",    Label = "Smooth Aim" },
...
local HudDiv = Instance.new("Frame")
HudDiv.Size = UDim2.new(1, 0, 0, 1)
HudDiv.BackgroundColor3 = Color3.fromRGB(120, 95, 185)
HudDiv.BackgroundTransparency = 0.6
HudDiv.BorderSizePixel = 0
HudDiv.Parent = Hud

local function HudInfo(label)
	local row = Instance.new("Frame")
	...
	value.Position = UDim2.new(1, -64, 0, 0)
...
local HudPartInfo = HudInfo("Aim Part")
local HudFovInfo = HudInfo("FOV")
local HudHitChance = HudInfo("Hit Chance")
local HudRageRate = HudInfo("Rage Rate")

local HudFoot = Instance.new("TextLabel")
HudFoot.Size = UDim2.new(1, 0, 0, 16)
HudFoot.BackgroundTransparency = 1
HudFoot.Text = "Right Shift - open/close menu"
HudFoot.TextColor3 = Theme.Dim
HudFoot.Font = Enum.Font.Gotham
HudFoot.TextSize = 9
HudFoot.TextXAlignment = Enum.TextXAlignment.Left
HudFoot.Parent = Hud

-- ============ INVENTORY OVERLAY (shows above the default backpack) ============
local CoreGui = game:GetService("CoreGui")

local function IsInventoryOpen()
	-- Default Roblox backpack lives in CoreGui while open
	local ok, bp = pcall(function() return CoreGui:FindFirstChild("Backpack") end)
	if ok and bp then
		local ok2, enabled = pcall(function() return bp.Enabled end)
		if ok2 and enabled then return true end
		local ok3, mf = pcall(function() return bp:FindFirstChild("MainFrame", true) end)
		if ok3 and mf and mf:IsA("Frame") then
			local ok4, vis = pcall(function() return mf.Visible end)
			if ok4 and vis then return true end
		end
	end
	-- Fallback: some games clone it into PlayerGui
	local plrGui = LocalPlayer:FindFirstChild("PlayerGui")
	if plrGui then
		for _, gui in ipairs(plrGui:GetChildren()) do
			if gui:IsA("ScreenGui") and gui.Name:lower():find("backpack") and gui.Enabled then
				return true
			end
		end
	end
...
if UseDrawing then
	InvOverlay.bg = Drawing.new("Square")
	InvOverlay.bg.Thickness = 1
	InvOverlay.bg.Color = Color3.fromRGB(13, 12, 20)
	InvOverlay.bg.Filled = true
	InvOverlay.bg.Transparency = 0.72
	InvOverlay.bg.Visible = false

	InvOverlay.accent = Drawing.new("Square")
	InvOverlay.accent.Thickness = 1.5
	InvOverlay.accent.Color = Theme.Accent
	InvOverlay.accent.Filled = false
	InvOverlay.accent.Transparency = 0.25
	InvOverlay.accent.Visible = false

	InvOverlay.title = Drawing.new("Text")
	InvOverlay.title.Text = "NUCLEAR.HOOK v2.3"
	InvOverlay.title.Size = 18
	InvOverlay.title.Color = Theme.Accent
	InvOverlay.title.Outline = true
	InvOverlay.title.OutlineColor = Color3.new(0, 0, 0)
	InvOverlay.title.Visible = false

	InvOverlay.sub = Drawing.new("Text")
	InvOverlay.sub.Text = "INVENTORY OPEN"
	InvOverlay.sub.Size = 11
	InvOverlay.sub.Color = Theme.Dim
	InvOverlay.sub.Outline = true
	InvOverlay.sub.OutlineColor = Color3.new(0, 0, 0)
	InvOverlay.sub.Visible = false
	...
	for _, def in ipairs(HudDefs) do
		...
		t.Visible = false
		InvOverlay.lines[def.Key] = t
	end

	InvOverlay.extra = Drawing.new("Text")
	InvOverlay.extra.Size = 11
	InvOverlay.extra.Color = Theme.Dim
	InvOverlay.extra.Outline = true
	InvOverlay.extra.OutlineColor = Color3.new(0, 0, 0)
	InvOverlay.extra.Visible = false

	local function SetInvOverlayVisible(show)
		InvOverlay.bg.Visible = show
		InvOverlay.accent.Visible = show
		InvOverlay.title.Visible = show
		InvOverlay.sub.Visible = show
		InvOverlay.extra.Visible = show
		for _, t in pairs(InvOverlay.lines) do t.Visible = show end
	end

	local wasOpen = false
	RunService.RenderStepped:Connect(function()
		local open = IsInventoryOpen()
		if open ~= wasOpen then
			wasOpen = open
			SetInvOverlayVisible(open)
		end
		if not open then return end

		local vp = Camera.ViewportSize
		local w, h = 260, 52 + #HudDefs * 20 + 22
		local x, y = vp.X - w - 14, 14

		InvOverlay.bg.Position = Vector2.new(x, y)
		InvOverlay.bg.Size = Vector2.new(w, h)
		InvOverlay.accent.Position = Vector2.new(x, y)
		InvOverlay.accent.Size = Vector2.new(w, h)

		InvOverlay.title.Position = Vector2.new(x + 12, y + 6)
		InvOverlay.sub.Position = Vector2.new(x + 12, y + 27)

		local yy = y + 50
		local active = 0
		for _, def in ipairs(HudDefs) do
			local t = InvOverlay.lines[def.Key]
			local on = _G[BindFlag[def.Key]] or IsBindDown(Binds[def.Key])
			t.Text = ("%s  [%s]"):format(def.Label, GetBindName(Binds[def.Key]))
			t.Color = on and Color3.fromRGB(90, 220, 140) or Theme.Dim
			t.Position = Vector2.new(x + 12, yy)
			yy = yy + 20
			if on then active = active + 1 end
		end

		InvOverlay.extra.Text = string.format("%d/%d active | Aim: %s | FOV: %dpx",
			active, #HudDefs, _G.AimPart, _G.AimFOV)
		InvOverlay.extra.Position = Vector2.new(x + 12, yy + 2)
	end)
end
...
if Watermark then
	Watermark.Text = "NUCLEAR.HOOK v2.3"
	Watermark.Size = 14
...
RunService.RenderStepped:Connect(function()
	...
	if Watermark then Watermark.Visible = _G.Watermark end
	...
	HudPartInfo.Text = _G.AimPart
	HudFovInfo.Text = tostring(_G.AimFOV) .. "px"
	HudHitChance.Text = tostring(_G.HitChance) .. "%"
	HudRageRate.Text = tostring(math.floor(_G.RageFireRate * 1000)) .. "ms"
end)
...
-- ANTI DROP-OFF: remember the last target
local lastTargetInfo = { plr = nil, aimPos = nil, lastSeen = 0 }
...
local function GetClosest(center)
	...
	for _, plr in ipairs(Players:GetPlayers()) do
		...
		if _G.VisCheck and not isVisible(aimPart) then
			-- With anti drop-off, consider players behind cover as candidates
		...
		if not onScreen then
			-- With anti drop-off, consider off-screen players
			if not (_G.AntiDropOff or IsBindDown(Binds.AntiDrop)) then continue end
			-- Use screen edge distance for off-screen targets
	...
	-- Anti Drop-Off: remember target even when lost for a brief moment
	...
	if closest then
		...
		if closest.Character then
			local ap = GetAimPart(closest.Character)
			lastTargetInfo.aimPos = ap and ap.Position or nil
		end
...
-- INSTANT HIT: common remote names across many games
...
local function FireInstantHit(target)
	...
	-- Try generic workspace remotes
	for _, name in ipairs(InstantHitRemotes) do
		for _, obj in ipairs(workspace:GetDescendants()) do
			if obj.Name == name and (obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction")) then
				pcall(function()
					if obj:IsA("RemoteEvent") then
						obj:FireServer(target.Character, target.Character:FindFirstChild("HumanoidRootPart"))
					else
						obj:InvokeServer(target.Character, target.Character:FindFirstChild("HumanoidRootPart"))
					end
				end)
			end
		end
...
RunService:BindToRenderStep("NuclearHookAim", Enum.RenderPriority.Camera.Value + 5, function()
	...
	local wantLock      = _G.LockAim      or IsBindDown(Binds.Lock)
	local wantSmooth    = _G.SmoothAim    or IsBindDown(Binds.Smooth)
	local wantSpin      = _G.Spinbot      or IsBindDown(Binds.Spin)
	local wantPred      = _G.Prediction   or IsBindDown(Binds.Pred)

	-- Override anti-drop if rage is active
	if wantRage then
		_G.AntiDropOff = true -- force anti-drop during rage
	end

	local target = GetClosest(center)

	-- For rage bot, always override with the closest target
	if wantRage and not target then
		-- Find ANY target, ignoring FOV
		target = GetClosest(center)
		-- If still nothing, widen search
		if not target then
			for _, plr in ipairs(Players:GetPlayers()) do
				if plr == LocalPlayer then continue end
				local char = plr.Character
	...
	if aimPos then
		if wantSilent then
			...
			if wantLock then
				...
				Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, aimPos)
			elseif wantSmooth then
				if Camera.CameraType == Enum.CameraType.Scriptable then
					Camera.CameraType = originalCameraType
				end
				local goal = CFrame.lookAt(Camera.CFrame.Position, aimPos)
				Camera.CFrame = Camera.CFrame:Lerp(goal, 0.18)
			else
				if Camera.CameraType == Enum.CameraType.Scriptable then
					Camera.CameraType = originalCameraType
				end
		...
		if wantRage and aimPos and target then
			...
			if now - lastRageFire >= _G.RageFireRate then
				...
				if math.random(1, 100) <= _G.HitChance then
					...
					-- Also try instant hit if enabled
					if wantInstant then
	...
	if wantSpin and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
		LocalPlayer.Character.HumanoidRootPart.CFrame *= CFrame.Angles(0, math.rad(16), 0)
	end
end)

-- Triggerbot (works as toggle OR while holding its bind)
local lastClick = 0
RunService.Heartbeat:Connect(function()
	...
	-- If rage bot is active, let it handle shooting instead
	local wantRage = _G.RageBot or IsBindDown(Binds.RageBot)
	...
	if t and t.Character then
		...
		if on and (Vector2.new(pos.X, pos.Y) - center).Magnitude < 24 then
			-- Check hit chance before firing
			if math.random(1, 100) <= _G.HitChance then
				pcall(function() mouse1click() end)
				-- Also fire instant hit remotes
				if wantInstant then
					FireInstantHit(t)
				end
			end
...
-- FLY
local flyBV, flyBG, flyChar = nil, nil, nil

local function StopFly()
	if flyBV then pcall(function() flyBV:Destroy() end) end
	if flyBG then pcall(function() flyBG:Destroy() end) end
	if flyChar then
		local hum = flyChar:FindFirstChildOfClass("Humanoid")
		if hum then pcall(function() hum.PlatformStand = false end) end
	end
	flyBV, flyBG, flyChar = nil, nil, nil
end
...
LocalPlayer.CharacterAdded:Connect(function(char)
	...
	if _G.Fly then
		task.delay(0.3, function()
			if _G.Fly and char == LocalPlayer.Character then StartFly(char) end
		end)
	end
end)
...
RunService.RenderStepped:Connect(function()
	local char = LocalPlayer.Character
	if _G.Fly then
		if (not flyBV or not flyBV.Parent) and char then StartFly(char) end
		if flyBV and flyBG and char then
...
RunService.RenderStepped:Connect(function()
	...
	for _, plr in ipairs(Players:GetPlayers()) do
		...
		if _G.ESP_Distance then
			o.dist.Text = math.floor((Camera.CFrame.Position - root.Position).Magnitude) .. " studs"
			o.dist.Color = Color3.fromRGB(200, 190, 230)
...
print("[Nuclear.Hook] v2.3 loaded | 10 aim binds | Anti Drop-Off + Instant Hit + Hit Chance + Rage Bot | Inventory Overlay")
